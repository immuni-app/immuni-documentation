...
**Figure 1**. Analytics tokens generation timeline.
...
**Figure 2**. The device successfully authorises an analytics token. Later, it uses the token to upload Operational Info with Exposure. It then generates and successfully authorises a new analytics token the next calendar month.
...
**Figure 3**. Sequence diagram representing the race condition attack mitigation in action.
